var Chart = React.createClass({
  render: function() {
    ...
    var x = d3.scale.linear()
      .domain([
        d3.min(data.map(function(d){ return dateFormat.parse(d.Date).getTime(); })),
        d3.max(data.map(function(d){ return dateFormat.parse(d.Date).getTime(); }))
      ])
      .range([margin,width - margin]);
    var y = d3.scale.linear()
      .domain([ d3.min(data.map(function(q){ return q.Low; })), d3.max(data.map(function(q){ return q.High; })) ])
      .range([height - margin, margin]);

    // calculate candle widths based on volume
    var totalVolume = _.reduce(data, function(memo, quote){ return memo + quote.Volume; }, 0);
    _.each(data, function(quote, i){
      if(this.props.fatStickMode) {
        quote.width = (width - 2 * margin) * (quote.Volume / totalVolume);        
      } else {
        quote.width = width / data.length;
      }
      if(i){
